...
def merge_tables(table1, table2, key):
...
def upload_to_database(df, db_host, db_user, db_password, db_name, table_name):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            # If the table already exists, prompt the user for another name
            if table_exists:
                new_table_name = input("The table already exists. Please enter another name: ")
                return upload_to_database(df, db_host, db_user, db_password, db_name, table_name=new_table_name)
...
async def cleanData(
    ...
    # Perform data cleaning operations based on the received parameters
    ...
    if table1_name and table2_name: #THIS IS NOT WORKING
        # Assuming table1 and table2 are already defined DataFrames
        table1 = get_table_data(table1_name)  # Get DataFrame for table1
        table2 = get_table_data(table2_name)  # Get DataFrame for table2
        df = merge_tables(table1, table2, merge_key)
    if remove_duplicates_key:  #THIS IS NOT WORKING
        df = remove_duplicates(df, remove_duplicates_key)
    
    
    # Add more conditions for other cleaning operations
    ...
    upload_to_database(df,db_host,db_user,db_password,db_name,table_name)
